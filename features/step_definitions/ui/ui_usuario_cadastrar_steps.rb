...
Entao('deverá redirecionar para a página de cadastro') do
  expect(page).to have_current_path('http://www.inmrobo.tk/accounts/signup/')
end

Entao('deverá retornar um modal de cadastro') do
...
Entao('deverá ser redirecionado para o login') do
  expect(page).to have_current_path('http://www.inmrobo.tk/accounts/login/')
end

Entao('deverá retornar um modal de login') do
  expect(@login.title_page.text).to eql('Login')
end

Entao('deverão retornar alertas de campo obrigatório') do
...
Entao('deverá retornar uma mensagem de erro do usuario {string}') do |message|
  expect(@register.text_erro_message).to eql(message)
end
...
Entao('deverá retornar uma mensagem {string}') do |message|
